...
const toString = (n: number) => {
  let s = ["", "", "", "", "", "", ""]; // too avoid HOLEY_ELEMENTS
  for (let i = 0; i < 7; i++) {
...
export const disposeId = (id: string) => ids.delete(id);
export const generateUniqueIds = <T extends string>(...keys: T[]): [Record<T, string>, () => void] => {
  const idset = keys.reduce((idset, key) => {
